...
# == Description
#
# Remove generated artifacts and restore project to barebones state
#
# == Metadata
#
...
# == Usage
#
# ./scripts/init/deinitialize-project.sh [options]
...
# ### `remove_path`
#
# Remove a file or directory if it exists. Logs pass/step accordingly.
remove_path() {
    local target="$1"
    if [[ -e "$target" ]]; then
        rm -rf "$target"
        log_pass "Removed $target"
    else
        log_step "$target already absent"
    fi
}

# ### `revert_file`
#
# Restore a tracked file to its committed state via git checkout.
revert_file() {
    local target="$1"
    if git diff --quiet -- "$target" 2>/dev/null; then
        log_step "$target already clean"
    else
        git checkout HEAD -- "$target"
        log_pass "Reverted $target"
    fi
}
...
main() {
    ...
    # Remove agent skills
    log_flow "Removing agent skills..."
    if cmd_exec bunx skills remove --all --yes; then
        log_pass "Skills unregistered"
    else
    ...
    local agent_dirs=(
        .adal .agent .augment .cline .codebuddy .commandcode .continue
        .crush .cursor .factory .goose .iflow .junie .kilocode .kiro
        .kode .mcpjam .mux .neovate .openhands .pi .pochi .qoder .qwen
        .roo .trae .vibe .windsurf .zencoder
    )
    for agent_dir in "${agent_dirs[@]}"; do
        [[ -d "$agent_dir/skills" ]] && remove_path "$agent_dir/skills" || true
        if [[ -d "$agent_dir" ]] && [[ -z "$(ls -A "$agent_dir")" ]]; then
            remove_path "$agent_dir"
        fi
    done
    # Claude Code skills live under .claude/skills (don't remove .claude itself)
    [[ -d .claude/skills ]] && remove_path .claude/skills || true
    log_done "Agent skills removed."
    ...
    revert_file src/app/globals.css
    revert_file package.json
    log_done "Files reverted."

    # Reinstall dependencies to sync lockfile
    log_flow "Syncing dependencies..."
    cmd_exec bun install
    log_done "Dependencies synced."

    log_done "Project deinitialized."
}
