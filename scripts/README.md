...
- **CLI flags** — [add_flag](scripts:meta/create-sh-script.sh#add_flag) / [parse_flags](scripts:meta/create-sh-script.sh#parse_flags) for declarative `--flag value` registration with auto-generated [usage](scripts:meta/create-sh-script.sh#usage). Flags pre-set variables so `ask_user_via_*` prompts are skipped in non-interactive mode.
