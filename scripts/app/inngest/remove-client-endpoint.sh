#!/usr/bin/env bash
# = remove-client-endpoint.sh
#
# == Description
#
# Remove a durable endpoint route for an Inngest client
#
# == Metadata
#
...
# ./scripts/app/inngest/remove-client-endpoint.sh  # interactive prompt
...
add_flag "path" "ROUTE_PATH" "Route path to remove (e.g. webhooks/stripe)"
...
# Find route.ts files that import from @/inngest/<client>/client.
# Returns paths relative to src/app/api/ (e.g. "webhooks/stripe").
discover_endpoints() {
    local client_id="$1"
    local pattern="@/inngest/${client_id}/client"
    local endpoints=()
    while IFS= read -r file; do
        local path="${file#src/app/api/}"
        path="${path%/route.ts}"
        endpoints+=("$path")
    done < <(grep -rl "$pattern" --include='route.ts' src/app/api/ 2>/dev/null || true)
    (( ${#endpoints[@]} )) || die "No endpoints found for client '$client_id'"
    printf '%s\n' "${endpoints[@]}"
}
...
main() {
    ...
    # Discover endpoints for this client
    local endpoints
    mapfile -t endpoints < <(discover_endpoints "$CLIENT_ID")

    if (( ${#endpoints[@]} == 1 )); then
        ROUTE_PATH="${ROUTE_PATH:-${endpoints[0]}}"
        log_step "Found endpoint: $ROUTE_PATH"
    else
        ask_user_via_select ROUTE_PATH "Which endpoint to remove?" "${endpoints[@]}"
    fi

    if ! ask_user_via_confirm CONFIRM "Remove endpoint '$ROUTE_PATH' from client '$CLIENT_ID'?" "n"; then
        log_dull "Aborted."
        return
    fi
    ...
    log_flow "Removing endpoint '$ROUTE_PATH' from client '$CLIENT_ID'..."

    if [[ -f "$route_file" ]]; then
        rm "$route_file"
        log_pass "Removed $route_file"
    else
        log_dull "Not found: $route_file (skipping)"
    fi

    # Prune empty parent directories
    local route_dir
    route_dir="$(dirname "$route_file")"
    prune_empty_parents "$route_dir" "src/app/api"

    log_done "Endpoint '$ROUTE_PATH' removed."
}
