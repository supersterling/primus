...
# == Description
#
# Remove an Inngest client and its API route
#
# == Metadata
#
...
# == Usage
#
# ./scripts/app/inngest/remove-client.sh --id <client-id>
# ./scripts/app/inngest/remove-client.sh  # interactive prompt
...
add_flag "id" "CLIENT_ID" "Client ID to remove (kebab-case)"
...
remove_dir() {
    ...
    if [[ -d "$dir" ]]; then
        ...
        log_dull "Not found: $dir/ (skipping)"
    fi
}
...
main() {
    ...
    ask_user_via_prompt CLIENT_ID "Client ID to remove (kebab-case)"

    local inngest_dir="src/inngest/$CLIENT_ID"
    local route_dir="src/app/api/inngest/$CLIENT_ID"

    if [[ ! -d "$inngest_dir" ]] && [[ ! -d "$route_dir" ]]; then
    ...
    log_flow "Removing inngest client '$CLIENT_ID'..."
