#!/usr/bin/env bash
# = remove-client-event.sh
#
# == Description
#
# Remove an event schema from an existing Inngest client's events.ts
#
# == Metadata
#
...
# ./scripts/app/inngest/remove-client-event.sh  # interactive prompt
...
main() {
    ...
    if (( ${#events[@]} == 1 )); then
        EVENT_ID="${EVENT_ID:-${events[0]}}"
        log_step "Found event: $EVENT_ID"
    else
    ...
    log_done "Event '$EVENT_ID' removed."
}
