...
> **Info:** You can define `{ enum: ["value1", "value2"] }` config to infer `insert` and `select` types, it **won't** check runtime values.
...
> **Info:** It's recommended to use `text('', { mode: 'json' })` instead of `blob('', { mode: 'json' })`,
