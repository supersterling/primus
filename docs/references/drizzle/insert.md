...
> **Info:** Check how to use WITH statement with [select](/docs/select#with-clause), [update](/docs/update#with-update-clause), [delete](/docs/delete#with-delete-clause)
...
> **Info:** The second form of the INSERT statement contains a SELECT statement instead of a VALUES clause.
...
> **Warning:** To avoid a parsing ambiguity, the SELECT statement should always contain a WHERE clause, even if that clause is simply "WHERE true", if the upsert-clause is present. Without the WHERE clause, the parser does not know if the token "ON" is part of a join constraint on the SELECT, or the beginning of the upsert-clause.

As the PostgreSQL documentation mentions:
> **Info:** A query (SELECT statement) that supplies the rows to be inserted

And as the MySQL documentation mentions:

> **Info:** With INSERT ... SELECT, you can quickly insert many rows into a table from the result of a SELECT statement, which can select from one or many tables
