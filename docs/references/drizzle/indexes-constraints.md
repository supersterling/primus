...
> **Info:** You can have many `UNIQUE` constraints per table, but only one `PRIMARY KEY` constraint per table.
...
> **Warning:** With MSSQL you can't create unique constraint on `text`, `ntext`, `varchar(max)`, `nvarchar(max)`
```typescript
...
```
> **Warning:** For versions before `drizzle-kit@0.22.0` and `drizzle-orm@0.31.0` `drizzle-kit` only supports index `name` and `on()` param.
...
```sql
    ...
    CREATE UNIQUE INDEX `email_idx` ON `user` (`email`);
```
> **Warning:** As of now `drizzle-kit` only supports index `name` and `on()` param.
...
> **Warning:** With MSSQL you can't create unique index on `text`, `ntext`, `varchar(max)`, `nvarchar(max)`
