...
> **Warning:** `offset` now can be used in with tables as well!
```typescript
await db.query.posts.findMany({
	limit: 5,
	offset: 2, // correct ✅
...
> **Info:** When you use multiple `orderBy` statements in the same table, they will be included in the query in the same order in which you added them
...
> **Warning:** If you will specify `.as("<alias>")` for any extras field - drizzle will ignore it
