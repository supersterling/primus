...
It's recommended to add this to the [`<ClerkProvider>`](https://clerk.com/docs/chrome-extension/reference/components/clerk-provider.md) that wraps your app so that it's applied to all Clerk components, as shown in the following example. The example names the layer `clerk` but you can name it anything you want.
