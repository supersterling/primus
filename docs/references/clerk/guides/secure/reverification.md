...
The simplest way to require reverification for a sensitive action is to use the [`useReverification()`](https://clerk.com/docs/nextjs/reference/hooks/use-reverification.md) hook. This hook will automatically check if the user has verified their credentials within 10 minutes and if not, displays a modal that prompts the user to verify their credentials. See the [`reference doc`](https://clerk.com/docs/nextjs/reference/hooks/use-reverification.md) for more information and comprehensive examples.
