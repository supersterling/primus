...
9. ## Create server actions for managing a user's Role
   ...
   1. In your `app/admin/` directory, create an `_actions.ts` file with the following code. The `setRole()` action checks that the current user has the `admin` Role before updating the specified user's Role using Clerk's [`JS Backend SDK`](https://clerk.com/docs/reference/backend/user/update-user.md). The `removeRole()` action removes the Role from the specified user.
