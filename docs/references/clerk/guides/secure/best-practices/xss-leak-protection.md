...
Clerk works to minimize the surface area by using `HttpOnly` cookies for authenticated requests to our [Frontend API](https://clerk.com/docs/reference/frontend-api.md){{ target: '_blank' }}, so that credentials cannot be leaked during XSS attacks.
