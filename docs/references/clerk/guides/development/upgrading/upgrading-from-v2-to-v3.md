...
[`useAuth()`](https://clerk.com/docs/nextjs/reference/hooks/use-auth.md) is a new, SSR-compatible hook that is recommended for all authentication tasks.

```jsx
const { userId, sessionId, getToken, isLoaded, isSignedIn, signOut } = useAuth()
```

The [`useAuth()`](https://clerk.com/docs/nextjs/reference/hooks/use-auth.md) hook unifies the way you access common auth operations. It replaces:
...
| `const session = useSession();` | `const { isLoaded, isSignedIn, session } = useSession();` <br />If you only call `useSession` for the ID or `getToken`, switch to [`useAuth()`](https://clerk.com/docs/nextjs/reference/hooks/use-auth.md) instead. |
...
Helpers for API routes have been updated to mirror the new [`useAuth()`](https://clerk.com/docs/nextjs/reference/hooks/use-auth.md) hook on the client-side.

- `withSession` is deprecated and replaced with `withAuth`
- `requireSession` is deprecated with replaced with `requireAuth`
- Instead of decorating the `Request` object with a `Session` object, it is now decorated with an `Auth` object that mirrors [`useAuth()`](https://clerk.com/docs/nextjs/reference/hooks/use-auth.md) on the client-side.
...
Edge middleware has also been updated to mirror the new [`useAuth()`](https://clerk.com/docs/nextjs/reference/hooks/use-auth.md) hook on the client-side. The import path has also been changed to avoid confusion.
...
Express middleware been updated to mirror the new [`useAuth()`](https://clerk.com/docs/nextjs/reference/hooks/use-auth.md) hook on the client-side.
