...
1. The [`useUser()`](https://clerk.com/docs/nextjs/reference/hooks/use-user.md) hook is used to get the current user's [`User`](https://clerk.com/docs/reference/javascript/user.md) object. The `isLoaded` boolean is used to ensure that Clerk is loaded.
...
3. The `addSSO()` function is used to add a new external account using the `strategy` that is passed in.
   - It uses the `user` object to access the [`createExternalAccount()`](https://clerk.com/docs/reference/javascript/user.md#create-external-account) method.
