...
- [Express & Clerk M2M Tokens Demo Repo](https://github.com/clerk/m2m-example/tree/main)
...
2. Under **Name**, enter your machine name (`Machine A`). Select **Create**.
...
4. In the list of machines, next to `Machine A`, select **Edit machine**.
...
To create an M2M token, call the [`createToken()`](https://clerk.com/docs/reference/backend/m2m-tokens/create-token.md) method:
...
Imagine a request was made from **Machine A** to **Machine B**, and Machine B received it successfully. The next step is to verify that the token is valid and that the request is coming from a valid machine. To do so, you can use the [`verifyToken()`](https://clerk.com/docs/reference/backend/m2m-tokens/verify-token.md) method:
...
To revoke an M2M token, call the [`revokeToken()`](https://clerk.com/docs/reference/backend/m2m-tokens/revoke-token.md) method:
