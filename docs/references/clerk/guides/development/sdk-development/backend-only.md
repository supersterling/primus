...
The source of truth for all BAPI endpoints is the [BAPI reference docs](https://clerk.com/docs/reference/backend-api.md){{ target: '_blank' }}. For Node.js backend frameworks, you can build on top of the [`JS Backend SDK`](https://clerk.com/docs/js-backend/getting-started/quickstart.md).
...
You can manually create a wrapper library around the [BAPI OpenAPI](https://clerk.com/docs/reference/backend-api.md){{ target: '_blank' }} or use one the many automatic SDK generation tools that take in OpenAPI definitions.
...
1. ### Create a Clerk client
   ...
   Use [`createClerkClient`](https://clerk.com/docs/nextjs/getting-started/quickstart.md#usage) from `@clerk/backend` to create your default Clerk client which will be used for the middleware.
