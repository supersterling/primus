...
- [Supabase, Next.js, and Clerk Demo](https://github.com/clerk/clerk-supabase-nextjs)
...
> This integration restricts what data authenticated users can access in the database, but does not synchronize user records between Clerk and Supabase. To send additional data from Clerk to your Supabase database, use [webhooks](https://clerk.com/docs/guides/development/webhooks/overview.md).
...
1. ## Set up Clerk as a Supabase third-party auth provider
   ...
   2. Choose your configuration options, and then select **Activate Supabase integration**. This will reveal the **Clerk domain** for your Clerk instance.
...
2. ## Set up RLS policies using Clerk session token data
...
5. ## Fetch Supabase data in your code
   ...
   **Client-side rendering**
   ...
   The following example uses the [`Next.js SDK`](https://clerk.com/docs/reference/nextjs/overview.md) to access the [`useUser()`](https://clerk.com/docs/nextjs/reference/hooks/use-user.md) and [`useSession()`](https://clerk.com/docs/nextjs/reference/hooks/use-session.md) hooks, but you can adapt this code to work with any React-based Clerk SDK.
