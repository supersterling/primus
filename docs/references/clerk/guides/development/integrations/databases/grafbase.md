...
4. ## Authenticate requests
   ...
   To retrieve Clerk's session token, use the `getToken()` method returned by the [`useAuth()`](https://clerk.com/docs/nextjs/reference/hooks/use-auth.md) hook. You'll need to pass the template name you created in the previous step to the `getToken()` method. The following example uses the default template name, `grafbase`.
