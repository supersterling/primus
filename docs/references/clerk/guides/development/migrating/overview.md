...
>
> Free plans have a limit of 50,000 MRUs per application, and once you exceed this limit you will be required to upgrade to the Pro plan. Head to our [pricing page](https://clerk.com/pricing) to get the full details on how Clerk charges.
...
You can programmatically retrieve a list of your user's and their information by using the [`GetUserList`](https://clerk.com/docs/reference/backend-api/tag/users/get/users.md){{ target: '_blank' }} Backend API endpoint, or the JS Backend SDK's [`getUserList()`](https://clerk.com/docs/reference/backend/user/get-user-list.md) method which is a wrapper around the Backend API endpoint.
