...
- Faster resolution of the `isLoaded` property and the [`<ClerkLoaded>`](https://clerk.com/docs/nextjs/reference/components/control/clerk-loaded.md) control component with only a single network fetch attempt. If the fetch fails, it gracefully falls back to cached resources.
- Network errors are no longer muted, allowing developers to catch and handle them effectively in their custom flows.
- The [`getToken()`](https://clerk.com/docs/reference/javascript/session.md#get-token) function in the `useAuth()` hook now supports returning cached tokens, minimizing disruptions caused by network failures.
...
2. ### Use the `__experimental_resourceCache` property on `ClerkProvider`
   ...
   On [`<ClerkProvider>`](https://clerk.com/docs/nextjs/reference/components/clerk-provider.md), pass the `resourceCache` object to the `__experimental_resourceCache` property, as shown in the following example:
