...
To customize the CAPTCHA widget, pass the `appearance` prop to the [`clerk()`](https://clerk.com/docs/reference/astro/overview.md) integration. The `appearance` prop accepts the property `captcha`, which can be used to apply different changes to the widget.
