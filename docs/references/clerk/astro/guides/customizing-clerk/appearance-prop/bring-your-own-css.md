...
It's recommended to add this to the [`clerk()`](https://clerk.com/docs/reference/astro/overview.md) integration in your `astro.config.mjs` file so that it's applied to all Clerk components, as shown in the following example. The example names the layer `clerk` but you can name it anything you want.
