...
![The inspect element tab opened with an element selected. It shows a list of classes and a lock icon in between human-readable classnames and randomly generated ones](https://clerk.com/docs/images/customization/identifying_elements.png)
...
It's recommended to add this to the [`defineNuxtConfig()`](https://clerk.com/docs/reference/nuxt/clerk-middleware.md) integration in your `nuxt.config.mjs` file so that it's applied to all Clerk components, as shown in the following example. The example names the layer `clerk` but you can name it anything you want.
