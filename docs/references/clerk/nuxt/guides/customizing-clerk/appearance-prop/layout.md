...
To make changes to the layout, pass the `appearance` prop to the [`defineNuxtConfig()`](https://clerk.com/docs/reference/nuxt/clerk-middleware.md) integration. The `appearance` prop accepts the property `layout`, which can be used to apply different changes to the widget.
