...
To customize the CAPTCHA widget, pass the `appearance` prop to the [`defineNuxtConfig()`](https://clerk.com/docs/reference/nuxt/clerk-middleware.md) integration. The `appearance` prop accepts the property `captcha`, which can be used to apply different changes to the widget.
