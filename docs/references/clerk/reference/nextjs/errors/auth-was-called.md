...
This error commonly appears when using Clerk with Next.js and the [`clerkMiddleware()`](https://clerk.com/docs/reference/nextjs/clerk-middleware.md) helper is configured _not_ to run on static asset routes, which is part of the default setup. It can happen for the following reasons:
...
The most common cause of this error is when [`auth()`](https://clerk.com/docs/reference/nextjs/app-router/auth.md) is called **without** a [`<ClerkProvider>`](https://clerk.com/docs/nextjs/reference/components/clerk-provider.md) wrapping the component tree where Clerk components are used.
...
Ensure that `<ClerkProvider>` is set up at a higher level in your component hierarchy than any Clerk components like [`<UserButton />`](https://clerk.com/docs/nextjs/reference/components/user/user-button.md), [`<SignedIn />`](https://clerk.com/docs/nextjs/reference/components/control/signed-in.md), or others. [`The Next.js quickstart`](https://clerk.com/docs/nextjs/getting-started/quickstart.md) shows how you can do this in your root layout.
...
6. Since `<ClerkProvider>` is usually placed in the root layout, it runs, but because the request didn't match the middleware matcher, `clerkMiddleware()` never executed. As a result, `<ClerkProvider>` attempts to call `auth()` **without context**, leading to one of these outcomes:
...
**Scope the <ClerkProvider> to a sub-layout**
