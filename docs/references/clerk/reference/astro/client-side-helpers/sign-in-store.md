...
The `$signInStore` store provides a convenient way to access the [`SignIn`](https://clerk.com/docs/reference/javascript/sign-in.md){{ target: '_blank' }} object, which allows you to check the current state of a sign-in. This is also useful for creating a custom sign-in flow.
...
The possible values for the `status` property of the `SignIn` resource are listed [`here`](https://clerk.com/docs/reference/javascript/sign-in.md#properties).
