...
You can customize Clerk components by passing an object of variables to the `variables` property of the [`appearance`](https://clerk.com/docs/fastify/guides/customizing-clerk/appearance-prop/overview.md) prop.
...
To apply `variables` to all Clerk components, pass the `appearance` prop to the [`clerkPlugin()`](https://clerk.com/docs/reference/fastify/clerk-plugin.md) integration. The `appearance` prop accepts the property `variables`.
...
> Please consider this approach with browser compatibility in mind, as it may not work in older browsers. See [`Browser Compatibility Considerations`](https://clerk.com/docs/fastify/guides/customizing-clerk/appearance-prop/variables.md#browser-compatibility-considerations) for details.
