...
To make changes to the layout, pass the `appearance` prop to the [`clerkPlugin()`](https://clerk.com/docs/reference/fastify/clerk-plugin.md) integration. The `appearance` prop accepts the property `layout`, which can be used to apply different changes to the widget.
