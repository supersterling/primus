...
The `<TaskSetupMFA />` component renders a UI for resolving the `setup-mfa` session task. You can further customize your `<TaskSetupMFA />` component by passing additional [`properties`](https://clerk.com/docs/react-router/reference/components/authentication/task-setup-mfa.md#properties) at the time of rendering.
