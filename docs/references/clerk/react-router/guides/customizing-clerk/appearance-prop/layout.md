...
To make changes to the layout, pass the `appearance` prop to the [`<ClerkProvider>`](https://clerk.com/docs/react-router/reference/components/clerk-provider.md) component. The `appearance` prop accepts the property `layout`, which can be used to apply different changes to the widget.
